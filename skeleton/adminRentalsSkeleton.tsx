...
export default function AdminRentalsSkeleton() {
  return (
    <div className="max-w-7xl mx-auto px-3.5 sm:px-6 py-5 sm:py-8 space-y-5 sm:space-y-6 animate-pulse">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-xs overflow-hidden">
        ...
        <div className="block md:hidden divide-y divide-slate-100 dark:divide-slate-800">
          {[...Array(5)].map((_, i) => (
            <div key={i} className="p-4 space-y-3">
              {/* Card Header: Property Title, Price & Status */}
              <div className="flex items-start justify-between gap-2">
                <div className="space-y-2 flex-1">
                  <div className="h-4 w-3/4 bg-slate-200 dark:bg-slate-800 rounded"></div>
                  <div className="h-3.5 w-16 bg-slate-200 dark:bg-slate-800 rounded ml-5"></div>
                </div>
                <div className="h-5 w-16 bg-slate-200 dark:bg-slate-800 rounded-md shrink-0"></div>
              </div>

              {/* Card Middle Box: Tenant & Payment Status */}
              ...
              {/* Card Footer: Property Availability */}
        ...
        <div className="flex items-center justify-between p-3.5 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/40">
          <div className="h-3.5 w-56 bg-slate-200 dark:bg-slate-800 rounded"></div>
